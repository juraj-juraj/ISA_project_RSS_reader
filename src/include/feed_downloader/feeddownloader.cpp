...
void feeddownloader::feedDownloader::httpsDownload(urlParser::URLAddress &address)
{
    ...
    if(res != 1)
        throw feedreaderException::downloader("Cannot set connection to server %s", address.address);

    BIO_get_ssl(web, &mSsl);
    if(mSsl == NULL)
        throw feedreaderException::downloader("Cannot get ssl connection to server %s", address.address);


    res = SSL_set_cipher_list(mSsl, mPREFERRED_CIPHERS);
    if(res != 1)
        throw feedreaderException::downloader("Cannot set prefered cipher list to server %s", address.address);

    res = SSL_set_tlsext_host_name(mSsl, address.address.c_str());
    if(res != 1)
        throw feedreaderException::downloader("Cannot set tlsext to server %s", address.address);

    out = BIO_new_fp(stdout, BIO_NOCLOSE);
    if(out == NULL)
        throw feedreaderException::downloader("Cannot make file pointer to server %s", address.address);

    res = BIO_do_connect(web);
    if(res != 1)
        throw feedreaderException::downloader("Cannot connect to server %s", address.address);

    res = BIO_do_handshake(web);
    if(res != 1)
        throw feedreaderException::downloader("Cannot estabilish handshake to server %s", address.address);
    ...
    if(cert == NULL)
        throw feedreaderException::downloader("Certificate from server %s is not valid", address.address);
...
void feeddownloader::feedDownloader::httpDownload(urlParser::URLAddress &address)
{
    ...
    if(res != 1)
        throw feedreaderException::downloader("Cannot do handshake to server %s", address.address);
...
void feeddownloader::feedDownloader::readFromBio(BIO *web, BIO *out)
{
    int len = 0;
    char buff[_downloaderconsts::BUFF_SIZE] = {0};
    do
